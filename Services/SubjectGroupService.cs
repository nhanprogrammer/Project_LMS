...
public class SubjectGroupService : ISubjectGroupService
{
        ...
        CreateSubjectGroupRequest createSubjectGroupRequest)
    {
        ...
        if (exitingSubjectGroup != null)
        {
            ...
            (
                1, "Subject Group đã tồn tại", null
            );
        }
    ...
    public async Task<ApiResponse<List<SubjectGroupDropdownResponse>>> GetSubjectGroupDropdownAsync()
    {
        var subjectGroups = await _context.SubjectGroups
            .Where(sg => sg.IsDelete == false)
            .Select(sg => new SubjectGroupDropdownResponse
            {
                Id = sg.Id,
                Name = sg.Name ?? string.Empty
            })
            .ToListAsync();

        if (!subjectGroups.Any())
        {
            return new ApiResponse<List<SubjectGroupDropdownResponse>>(1, "Không có nhóm môn học nào.", null);
        }

        return new ApiResponse<List<SubjectGroupDropdownResponse>>(0, "Lấy danh sách nhóm môn học thành công.", subjectGroups);
    }
